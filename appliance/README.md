...
| `GATEWAY_CONTROL_TOKEN` | `''` (loopback-only) | shared secret that lets an authenticated **remote** GUI manage control-plane routes; unset keeps those routes loopback-only (see "Off-device control" below) |
...
## Off-device control (remote GUI to remote gate)

The gate and its GUI need not share a machine, but remote management is an explicit security opt-in. The default bind is loopback and remains the recommended local posture.

- **Redaction traffic and management are different boundaries.** A non-loopback `GATEWAY_HOST` makes `/v1/*` relay routes reachable to network peers. `GATEWAY_CONTROL_TOKEN` does **not** authenticate those relay routes. Do not expose that listener to the public internet.
- **Control-plane routes are local by default.** Remote access to control-plane routes such as `/api/*` and `/gate/*` requires a configured `GATEWAY_CONTROL_TOKEN` and the `X-OSSRedact-Control-Token` request header. `GET /api/claude_cli/bootstrap` is an explicit client-configuration passthrough, not a redaction or remote-control route. `/healthz` is public but grants no control. The gate-served `/` and `/console` stay loopback-only even when remote control is enabled.
- **Configure remote control deliberately.** Bind `GATEWAY_HOST` only to a trusted interface and set `GATEWAY_CONTROL_TOKEN=<synthetic-control-token>` in an owner-only environment file. Every remote control read, including fetch-based SSE at `/api/stream`, carries `X-OSSRedact-Control-Token` in a header. URL query credentials are never accepted. State-changing control routes also require `X-OSSRedact-Control: 1`.
- **Keep the operator token ephemeral.** The desktop console holds the control token only in session memory, never in local storage or a URL. Its live-stream reconnect sends the header again while the session remains active; an app restart or browser reload requires re-entry.
- **Use an authenticated encrypted transport.** The proxy itself does not provide TLS. Use a trusted encrypted network and HTTPS or an equivalent authenticated secure transport for every off-device connection, including the real-PII live feed. A browser-served remote console also needs its exact origin in `GATEWAY_CORS_ORIGINS`; that CORS setting never bypasses the token.
- **Treat remote control as exposure of sensitive proof data.** With `GATEWAY_CONTROL_TOKEN` unset, remote control remains unavailable. Enabling it permits a token holder on the protected transport to read the in-memory live proof feed, which can contain real PII.
