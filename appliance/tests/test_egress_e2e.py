...
# The gate NER tags the WHOLE absolute path as file_path (verified live against gx10 :8001). These tests mock that
...
def _abs_path_detector():
    """Tag unix, Windows-drive, and tilde absolute paths as one file_path span each (as the gx10 NER does)."""
    async def detector(text, min_score=0.5):
        return [{'start': m.start(), 'end': m.end(), 'label': 'file_path', 'tier': 1, 'conf': 0.99, 'rule': 'npu'}
...
# ---------------------------------------------------------------------------
# GATEWAY_GATE_TOKEN: optional egress->gate auth header on outbound /detect only.
# Shared with a protected remote primary/fallback; loopback gates ignore it.
# Wrong/missing token on a protected gate is an HTTP fault -> degrade (None) ->
# existing fail-closed surface (not a silent Tier-0-only forward).
# ---------------------------------------------------------------------------
@_NEEDS_PROXY
def test_gateway_gate_token_attached_only_on_detect_post(monkeypatch):
    """When GATEWAY_GATE_TOKEN is set, every egress->gate /detect POST carries
    X-OSSRedact-Gate-Token. When unset, the header is absent. Never invent a
    per-route token format; one shared secret for primary + fallback.
    """
    monkeypatch.setattr(egress_proxy, '_DETECT_CACHE', egress_proxy.OrderedDict())
    monkeypatch.setattr(egress_proxy, 'GATE_URL', 'http://remote-gate:8001')
    monkeypatch.setattr(egress_proxy, 'GATE_FALLBACK_URL', '')
    ...
    class _CaptureClient:
        async def post(self, url, json=None, headers=None, **kwargs):
            captured.append({'url': url, 'json': json, 'headers': dict(headers or {})})
            # Minimal successful gate response so _detect_against returns spans.
            class _R:
    ...
    # Token SET: header must be present on /detect.
    monkeypatch.setattr(egress_proxy, 'GATEWAY_GATE_TOKEN', 'synth-gate-token-001', raising=False)
    asyncio.run(egress_proxy._detect_against(_CaptureClient(), egress_proxy.GATE_URL, 'synthetic field text', 0.5))
    assert captured, 'expected at least one /detect POST'
    for call in captured:
        assert call['url'].endswith('/detect'), call['url']
        assert call['headers'].get('X-OSSRedact-Gate-Token') == 'synth-gate-token-001', (
            f'GATEWAY_GATE_TOKEN must be sent as X-OSSRedact-Gate-Token on /detect; got {call["headers"]!r}')

    # Token UNSET: header must be absent (loopback/unprotected gates stay header-free).
    captured.clear()
    monkeypatch.setattr(egress_proxy, 'GATEWAY_GATE_TOKEN', '', raising=False)
    asyncio.run(egress_proxy._detect_against(_CaptureClient(), egress_proxy.GATE_URL, 'synthetic field text', 0.5))
    assert captured, 'expected at least one /detect POST without token'
    for call in captured:
        assert 'X-OSSRedact-Gate-Token' not in call['headers'], (
            'unset GATEWAY_GATE_TOKEN must not attach an auth header on /detect')


@_NEEDS_PROXY
def test_gateway_gate_token_missing_or_wrong_is_fail_closed(monkeypatch):
    """A protected remote gate rejecting the token (HTTP 401/403) is a reachable-gate
    HTTP fault, not a transport outage: _detect_neural returns None (no fallback
    paper-over), and redact_body flags degraded so the existing fail-closed 503
    surface refuses to forward. GATEWAY_GATE_TOKEN is distinct from GATEWAY_CONTROL_TOKEN.
    """
    monkeypatch.setattr(egress_proxy, '_DETECT_CACHE', egress_proxy.OrderedDict())
    monkeypatch.setattr(egress_proxy, 'GATE_URL', 'http://remote-gate:8001')
    monkeypatch.setattr(egress_proxy, 'GATE_FALLBACK_URL', 'http://127.0.0.1:8001')
    monkeypatch.setattr(egress_proxy, 'GATEWAY_GATE_TOKEN', 'wrong-or-missing-token', raising=False)
    monkeypatch.setattr(egress_proxy, 'FAIL_CLOSED', True)

    # Simulate a protected gate that rejects the presented (or missing) token with 401.
    # HTTPStatusError is NOT a TransportError, so failover must NOT fire.
    async def _rejecting_detect_against(aclient, gate_url, text, min_score):
        raise egress_proxy.httpx.HTTPStatusError('401 unauthorized gate token')

    monkeypatch.setattr(egress_proxy, '_detect_against', _rejecting_detect_against)
    spans = asyncio.run(egress_proxy._detect_neural(None, 'Jean lives here', 0.5))
    assert spans is None, (
        'a protected gate rejecting GATEWAY_GATE_TOKEN must degrade (None), not failover or crash')

    # End-to-end: degraded neural path under FAIL_CLOSED refuses the forward.
    # Drive redact_body with the same None neural so meta['degraded'] is set, then
    # the route-level _degraded_block returns 503.
    async def _none(aclient, text, min_score=0.5):
        return None

    monkeypatch.setattr(egress_proxy, '_detect_neural', _none)
    body = {'model': 'claude-test',
            'messages': [{'role': 'user', 'content': 'Notify synthetic person Alice Proof today.'}]}
    meta, _replay = asyncio.run(
        egress_proxy.redact_body(body, {'session': 'tok-fc-' + os.urandom(4).hex(), 'project': 'e2e'}))
    assert meta.get('degraded') is True, 'unreachable/rejected gate must flag degraded'
    blocked = egress_proxy._degraded_block(meta)
    assert blocked is not None, 'fail-closed must refuse forward when gate token is rejected'
    assert _response_status(blocked) == 503, 'existing fail-closed surface is HTTP 503'


# ---------------------------------------------------------------------------
# AGGREGATE DETECT CONCURRENCY: DETECT_CONCURRENCY bounds in-flight neural
# calls across concurrent requests (module-scope semaphore), not just inside
# one redact_body. Deterministic fi-order minting remains covered by
# test_concurrent_detection_deterministic_minting.
# ---------------------------------------------------------------------------
@_NEEDS_PROXY
def test_detect_concurrency_global_bound_across_requests(monkeypatch):
    """Overlapping redact_body calls must never exceed DETECT_CONCURRENCY neural
    entries in flight at once. A per-request semaphore would allow N_requests *
    DETECT_CONCURRENCY simultaneous model calls; the bound is process-global.
    """
    bound = 2
    monkeypatch.setattr(egress_proxy, 'DETECT_CONCURRENCY', bound)
    # Force a fresh module-scope semaphore if the implementation exposes one;
    # production must share it. If absent, the test fails by observing overshoot.
    if hasattr(egress_proxy, 'DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, 'DETECT_SEMAPHORE', asyncio.Semaphore(bound))
    elif hasattr(egress_proxy, '_DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, '_DETECT_SEMAPHORE', asyncio.Semaphore(bound))

    inflight = {'n': 0, 'max': 0}
    lock = asyncio.Lock()

    async def slow_detector(text, min_score=0.5):
        async with lock:
            inflight['n'] += 1
            if inflight['n'] > inflight['max']:
                inflight['max'] = inflight['n']
        try:
            # Hold the slot long enough for sibling requests to race the semaphore.
    ...
    class NoHttpxClient:
        def __init__(self, *a, **k):
            raise AssertionError('injected detector must not construct httpx')

    monkeypatch.setattr(egress_proxy.httpx, 'AsyncClient', NoHttpxClient)

    # Several concurrent requests, each with several scannable fields, so a
    # per-request semaphore would overshoot the global bound.
    n_requests = 4
    fields_per = 4

    async def one(i):
        body = {'model': 'claude-test', 'messages': [
            {'role': 'user', 'content': f'req{i} field{j} synthetic value {j} here'}
            for j in range(fields_per)]}
        ctx = {'session': f'conc-bound-{i}-' + os.urandom(3).hex(), 'project': 'e2e'}
        return await egress_proxy.redact_body(body, ctx, detector=slow_detector)
    ...
    assert inflight['max'] > 0, 'detector must have been entered'
    assert inflight['max'] <= bound, (
        f'aggregate in-flight neural calls peaked at {inflight["max"]}, '
        f'exceeding DETECT_CONCURRENCY={bound} (semaphore must be module-scoped)')


@_NEEDS_PROXY
def test_detect_concurrency_preserves_deterministic_minting_under_global_bound(monkeypatch):
    """Global concurrency must not regress fi-order minting: serial bound=1 and
    concurrent bound=2 over the same multi-field body yield identical wire bytes
    and replay maps (prompt-cache stability). Complements the reordering test.
    """
    def build():
        return {'model': 'claude-x', 'messages': [
            {'role': 'user', 'content': f'msg {i}: email person{i}@example.com re Client Name{i}'}
            for i in range(8)]}

    found = {f'person{i}@example.com': 'email' for i in range(8)}
    found.update({f'Client Name{i}': 'person' for i in range(8)})
    base = _make_neural(found)

    async def reordering(aclient, text, min_score=0.5):
        m = re.search(r'msg (\d+):', text)
        if m:
            await asyncio.sleep((8 - int(m.group(1))) * 0.001)
        return await base(aclient, text, min_score)

    monkeypatch.setattr(egress_proxy, 'DETECT_CONCURRENCY', 1)
    if hasattr(egress_proxy, 'DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, 'DETECT_SEMAPHORE', asyncio.Semaphore(1))
    elif hasattr(egress_proxy, '_DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, '_DETECT_SEMAPHORE', asyncio.Semaphore(1))
    b1 = build()
    _m1, r1 = _run_redact(monkeypatch, b1, base,
                          ctx={'session': 'gbound-serial-' + os.urandom(4).hex(), 'project': 'e2e'})

    monkeypatch.setattr(egress_proxy, 'DETECT_CONCURRENCY', 2)
    if hasattr(egress_proxy, 'DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, 'DETECT_SEMAPHORE', asyncio.Semaphore(2))
    elif hasattr(egress_proxy, '_DETECT_SEMAPHORE'):
        monkeypatch.setattr(egress_proxy, '_DETECT_SEMAPHORE', asyncio.Semaphore(2))
    b2 = build()
    _m2, r2 = _run_redact(monkeypatch, b2, reordering,
                          ctx={'session': 'gbound-conc-' + os.urandom(4).hex(), 'project': 'e2e'})

    assert _wire_text(b1) == _wire_text(b2), (
        'global concurrency bound must preserve deterministic redacted bytes')
    assert r1 == r2, 'global concurrency bound must preserve deterministic replay maps'
