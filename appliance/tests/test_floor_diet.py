...
def test_tier0_floor_still_wins_merge_over_demoted_model_span():
    ...
    # ...but the tier-0 floor wins the cluster. Since the 2026-07-07 cue_digit backstop, the primary is
    # account_number (the acct-cue rule outscores the generic context-cue sensitive_account_id span);
    # both are FLOOR members, so the invariant under test -- floor beats demoted model span -- is intact.
    assert [s['label'] for s in merged] in (['account_number'], ['sensitive_account_id'])
    assert merged[0]['label'] in egress_proxy.FLOOR_LABELS
