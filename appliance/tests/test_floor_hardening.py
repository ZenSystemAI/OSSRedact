...
def test_rc2_glued_bn_sin_cue_override_emits_government_id():
    # Phase-1 never-leak on the glued path: the same Luhn BN-looking value preceded by a SIN cue must emit
    # government_id via tier0:digit_glued (safe over-redaction). Synthetic public SIN test vector only.
    t = 'NAS client046454286RT0001'
    hits = [s for s in pg.tier0_spans(t) if s['rule'] == 'tier0:digit_glued' and s['label'] == 'government_id']
    assert hits, t
    assert t[hits[0]['start']:hits[0]['end']] == '046454286'
    assert hits[0].get('validator') == 'luhn_ok'
    assert hits[0].get('conf') == 0.8
