...
def test_query_token_is_stream_only(monkeypatch):
    # The leakier ?token= path is restricted to the SSE feed (which can't set a header). A NON-stream control
    # route must NOT authenticate via query param -- it requires the header. (/api/stream's 200 path streams,
    # so its allow-via-query is asserted at the helper level in test_control_allowed_helper_matrix.)
    monkeypatch.setattr(ep, 'CONTROL_TOKEN', _TOKEN)
    c = _remote_client()
    assert c.get(f'/api/live/status?token={_TOKEN}').status_code == 403   # query token NOT honored here
    assert c.get('/api/live/status', headers={'x-ossredact-control-token': _TOKEN}).status_code == 200  # header is
...
def test_control_allowed_helper_matrix(monkeypatch):
    ...
    # query token: honored ONLY on the SSE feed, rejected on every other control route
    assert ep._control_allowed(_Req('10.0.0.9', qs={'token': _TOKEN}, path='/api/stream'))
