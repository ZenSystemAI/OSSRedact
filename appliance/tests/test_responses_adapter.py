...
_OA_PATH = os.path.join(_APPLIANCE, 'openai_adapter.py')
_oa_spec = importlib.util.spec_from_file_location('openai_adapter_under_test', _OA_PATH)
...
# ---------------------------------------------------------------------------
# OPENAI CHAT LEAK PATH: an assistant `messages[].refusal` string is model-visible
# conversation history and must be surfaced exactly like assistant content.
# ---------------------------------------------------------------------------
def test_openai_chat_message_refusal_redacted():
    email = 'victoria.lavoie@example.test'
    ph = '<EMAIL_001>'
    body = {
        'model': 'gpt-test',
        'messages': [
            {'role': 'assistant',
             ...
             'refusal': f'I cannot share records for {email}.'},
        ],
    }

    fields = oa.extract_text_fields_openai(body)
    assert _swap_all(fields, email, ph) == 1, 'messages[].refusal must be surfaced for redaction'

    wire = json.dumps(body, ensure_ascii=False)
    assert email not in wire, 'PRIVACY FAILURE: raw value in messages[].refusal leaked upstream'
    assert ph in wire
...
def _prose_predicate_kinds_from_redact_body_source(source):
    """Return the literal kind allow-list used by redact_body's `prose = f.kind in (...)` predicate."""
    tree = ast.parse(textwrap.dedent(source))
    redact_body = next(
        (node for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and
         node.name == 'redact_body'),
        None)
    assert redact_body is not None, 'could not locate egress_proxy.redact_body'
    for node in ast.walk(redact_body):
        if not isinstance(node, ast.Assign):
            continue
        if not any(isinstance(target, ast.Name) and target.id == 'prose' for target in node.targets):
            continue
        checks = node.value.values if isinstance(node.value, ast.BoolOp) and isinstance(node.value.op, ast.And) else [node.value]
        for check in checks:
            if not (isinstance(check, ast.Compare) and len(check.ops) == 1 and isinstance(check.ops[0], ast.In)):
                continue
            left = check.left
            if not (isinstance(left, ast.Attribute) and left.attr == 'kind' and
                    isinstance(left.value, ast.Name) and left.value.id == 'f'):
                continue
            allowed = check.comparators[0]
            if isinstance(allowed, (ast.Tuple, ast.List, ast.Set)):
                return {
                    elt.value for elt in allowed.elts
                    if isinstance(elt, ast.Constant) and isinstance(elt.value, str)
                }
    raise AssertionError('could not locate the prose predicate in egress_proxy.redact_body')
...
def test_egress_prose_gate_includes_system_kind():
    ...
    if eg is not None:
        ...
        source = inspect.getsource(eg.redact_body)
    else:
        source = open(os.path.join(_APPLIANCE, 'egress_proxy.py'), encoding='utf-8').read()

    kinds = _prose_predicate_kinds_from_redact_body_source(source)
