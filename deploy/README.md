...
- `qc-pii-gate-cpu.service` -- systemd unit (port 8011, `CUDA_VISIBLE_DEVICES=` to stay off GPUs).
- `export_quantize_v11_cpu.py` -- exports an xlm-r fp32 checkpoint to ONNX, then **dynamic**
  (weights-only) INT8. Host paths are hard-coded to P620; edit `MDIR`/`CALIB` for another model.
...
```bash
# 1. EXPORT + QUANTIZE (CPU-only; on the box that holds the fp32 checkpoint, e.g. P620)
...
# 3. DEPLOY (the CPU tier belongs on the Beelink sidecar box, alongside the NPU tier)
...
sudo cp deploy/qc-pii-gate-cpu.service /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now qc-pii-gate-cpu.service
curl -s http://127.0.0.1:8011/healthz
```

## Deployment record (2026-06-16)
Live on **Beelink** (the sidecar box). The existing `qc-pii-gate.service` (:8001) was updated from
**v7 OpenVINO-FP16 on the Intel NPU** to **v11r5-base ONNX-INT8 on CPU** via the systemd drop-in
`qc-pii-gate.service.d/v11int8.conf` (repoints `ExecStart` to `gate_service_cpu.py`, sets
...
sudo rm /etc/systemd/system/qc-pii-gate.service.d/v11int8.conf
sudo systemctl daemon-reload && sudo systemctl restart qc-pii-gate.service
```
...
P620 validation copy of this tier was torn down (one gate, the best model).
