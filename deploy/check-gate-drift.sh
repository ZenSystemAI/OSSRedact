...
#
# SCOPE: check the files the target host actually RUNS. A remote GPU/CPU gate host runs ONLY the gate/
# detect services (gate_service_{gpu,cpu}.py, which import gate/privacy_gate.py and gate/gate_http_policy.py);
# it does NOT run the appliance egress proxy. The egress proxy runs on the WORKSTATION off the repo working tree directly (ExecStart points at
...
#
# The remote mirrors the repo layout under a configured base directory, so the SAME repo-relative path is
# compared on both sides.
#
#   # Select the gate host explicitly:
#   GATE_HOST=gate.example.invalid deploy/check-gate-drift.sh
#   # A host with an alternate file set:
#   GATE_HOST=alternate-gate.example.invalid GATE_REMOTE_BASE=alternate/ossredact GATE_FILES="gate/gate_service_cpu.py gate/privacy_gate.py gate/gate_http_policy.py" deploy/check-gate-drift.sh
#
# Exit 0 = in sync; non-zero = drift or an unreachable/missing file (reconcile before relying on that gate).
set -uo pipefail
if [ -z "${GATE_HOST:-}" ]; then
  printf '%s\n' 'GATE_HOST must be set to a nonempty SSH host.' >&2
  exit 2
fi
