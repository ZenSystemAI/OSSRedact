...
from privacy_gate import cue_digit_spans, cue_name_spans, validated_floor  # noqa: E402
...
def _spans(text):
    ...
    return [(s['label'], text[s['start']:s['end']]) for s in validated_floor(text) + cue_name_spans(text) + cue_digit_spans(text)]
