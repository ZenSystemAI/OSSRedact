...
import { FLOOR_LABELS, LABEL_REGISTRY, labelTier } from './labels'
...
describe('label registry contract', () => {
  ...
  it('keeps non-catastrophic shipped labels in the operational tier', () => {
    const wrongTier = [...shippedLabels]
      .filter((label) => !CATASTROPHIC_SHIPPED_LABELS.has(label) && labelTier(label) !== 'operational')
      .sort()
    expect(wrongTier).toEqual([])
  })

  it('keeps every hard-floor label in the catastrophic tier', () => {
    const wrongTier = [...FLOOR_LABELS].filter((label) => labelTier(label) !== 'catastrophic').sort()
    expect(wrongTier).toEqual([])
  })

  it('defaults unknown labels to catastrophic', () => {
    expect(labelTier('future_detector_label')).toBe('catastrophic')
  })
})
