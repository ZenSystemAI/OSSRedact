...
const CATASTROPHIC = new Set<string>([
  ...
  'sensitive_account_id', 'bank_account', 'routing_number', 'tax_id', 'secret', 'password',
  'api_key', 'access_token', 'email', 'person', 'date_of_birth',
...
export function labelTier(label: string): Tier {
  if (CATASTROPHIC.has(label)) return 'catastrophic'
  // default to catastrophic for unknown labels: over-redaction is the safe error for a privacy gate
  return Object.prototype.hasOwnProperty.call(LABEL_REGISTRY, label) ? 'operational' : 'catastrophic'
}
