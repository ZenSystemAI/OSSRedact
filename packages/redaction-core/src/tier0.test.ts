...
describe('tier0Spans', () => {
  ...
  it('amount-bleed regression: detects account number but NOT transaction amounts', () => {
    const text = 'compte 55566677788 2.75 $ 614.20 $'
    const spans = labeledSpans(text)

    const acctSpan = spans.find((s) => s.sub.replace(/\D/g, '') === '55566677788')
    ...
    expect(digits).toBe('55566677788')
  })
...
describe('tier0Spans glued digit-run floor', () => {
  ...
  it('suppresses a glued Luhn BN program account without a SIN cue (RT0001)', () => {
    // Phase-1 contract: gluedDigitSpans must mirror Python BN suppression. Public Luhn SIN vector
    // 046454286 + RT0001 is a GST/QST registration, not a personal SIN, so digit_glued must not emit.
    const spans = labeledSpans('ref046454286RT0001')
    expect(spans.some((s) => s.rule === 'tier0:digit_glued')).toBe(false)
    expect(spans.some((s) => s.label === 'government_id')).toBe(false)
  })

  it('emits glued government_id when a SIN cue precedes the identical BN-looking value', () => {
    // Never-leak override on the glued path: preceding SIN cue forces emission even with RT0001 suffix.
    const text = 'NAS client046454286RT0001'
    const hit = labeledSpans(text).find((s) => s.rule === 'tier0:digit_glued')
    expect(hit).toBeDefined()
    expect(hit!.label).toBe('government_id')
    expect(hit!.conf).toBe(0.8)
    expect(hit!.validator).toBe('luhn_ok')
    expect(text.slice(hit!.start, hit!.end)).toBe('046454286')
  })
})
...
describe('tier0Spans card aux (CVV + expiry)', () => {
  ...
  it('catches JSON-quoted CVV with quote-tolerant separators and underscore cues', () => {
    // Phase-1: port Python _QSEP / security_code tolerance. Synthetic 3-digit values only.
    for (const [text, value] of [
      ['{"cvv": 834}', '834'],
      ['{"cvv":"401"}', '401'],
      ['config = {"security_code": 720, "x": 1}', '720'],
    ] as const) {
      const hit = labeledSpans(text).find((s) => s.label === 'card_cvv' && s.sub === value)
      expect(hit, text).toBeDefined()
      expect(hit!.rule).toBe('tier0:cvv')
      expect(hit!.conf).toBe(0.9)
      expect(text.slice(hit!.start, hit!.end)).toBe(value)
    }
  })

  it('catches JSON-quoted card expiry with quote-tolerant separators', () => {
    const text = '{"expiry": "08/27"}'
    const hit = labeledSpans(text).find((s) => s.label === 'card_expiry')
    expect(hit).toBeDefined()
    expect(hit!.rule).toBe('tier0:expiry')
    expect(hit!.conf).toBe(0.9)
    expect(text.slice(hit!.start, hit!.end)).toBe('08/27')
  })

  it('catches cue-anchored numeric secrets (PIN/NIP/passcode/OTP) as password', () => {
    // Phase-1: emit FLOOR password for numeric secrets; bare numbers stay clean.
    for (const [text, value] of [
      ['{"pin": 5571}', '5571'],
      ['PIN: 1234', '1234'],
      ['NIP 4821', '4821'],
      ['otp 991823', '991823'],
      ['passcode: 84217', '84217'],
      ['{"account_pin": 5571}', '5571'],
      ['one-time code 553201', '553201'],
      ['access_code 448821', '448821'],
    ] as const) {
      const hit = labeledSpans(text).find((s) => s.label === 'password' && s.sub === value)
      expect(hit, text).toBeDefined()
      expect(hit!.rule).toBe('tier0:num_secret')
      expect(hit!.conf).toBe(0.9)
      expect(text.slice(hit!.start, hit!.end)).toBe(value)
    }
  })

  it('does NOT fire numeric-secret on bare numbers or non-cue word forms', () => {
    for (const text of ['pinned 5 items', 'spinning at 4500 rpm', 'shipping 250 units', 'there were 5571 rows']) {
      expect(labeledSpans(text).some((s) => s.rule === 'tier0:num_secret'), text).toBe(false)
      expect(labeledSpans(text).some((s) => s.label === 'password'), text).toBe(false)
    }
  })
})
...
describe('tier0Spans control-char (Cc) resistance', () => {
  const US = '\x1f' // UNIT SEPARATOR (category Cc), not covered by normSpace
