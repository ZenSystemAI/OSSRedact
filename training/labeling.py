...
or None (O). Labels not in `canon` are skipped (treated as O).
 ...
 - token_char_label: shared token-offset lookup that skips token-internal leading whitespace.
"""
from __future__ import annotations

def token_char_label(text, charlab, start, end):
    """Return the first non-whitespace character's label in a token range."""
    bound = min(len(text), len(charlab))
    start = min(max(start, 0), bound)
    end = min(max(end, 0), bound)
    for index in range(start, end):
        if not text[index].isspace():
            return charlab[index]
    return None
