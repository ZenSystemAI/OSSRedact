...
CATASTROPHIC = {"government_id", "payment_card", "card_cvv", "card_expiry", "secret", "password",
                "account_number", "iban", "sensitive_account_id", "email", "person", "date_of_birth", "tax_id"}
...
def check(path):
    ...
    for lab in sorted(pl):
        d = pl[lab]; cat = lab in CATASTROPHIC; flag = []
        if cat:
        ...
        print(f"{lab:22} {'CAT' if cat else 'op':4} {d['gold']:>6} {d['recall_labeled']:>7.3f} "
              f"{d['precision']:>7.3f} {d['f1']:>7.3f}  {','.join(flag)}")

    cat = [pl[l] for l in CATASTROPHIC if l in pl]
    cg = sum(d['gold'] for d in cat) or 1
    cr = sum(d['gold'] * d['recall_labeled'] for d in cat) / cg
    cp_den = sum(d['pred'] for d in cat) or 1
    cp = sum(d['pred'] * d['precision'] for d in cat) / cp_den
