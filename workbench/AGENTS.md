# AGENTS.md -- workbench (client-side redaction app)

The **app domain** of OSSRedact. Working here you do NOT need the model/training
...
## What this is
A local-first PII-redaction tool: load a `.txt` / `.docx` / `.xlsx` / `.pdf`, detect spans
(offline Tier-0 regex/checksum detectors, optionally augmented by the local gate's NER endpoint
or the hosted-demo browser model),
...
(and vice versa). Durable fix is direction **D1** (codegen from one source).
