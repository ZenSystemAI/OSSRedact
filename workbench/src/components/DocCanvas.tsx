...
// original substring 1:1, so base + in-node offset is exact. Active redaction chips render a same-length mask
// but still snap selection to chip boundaries so reviewers cannot select masked internals.
  ...
  return (
    ...
    >
      {segs.map((seg, i) => {
        ...
        const placeholder = placeholderOf.get(s.id) ?? `<${s.label.toUpperCase()}>`
        const common = {
          ...
          title: `${meta.en} · ${s.source} · ${s.rule} · ${placeholder}`,
          style: { ['--tok-color' as string]: meta.color, padding: 0, fontFamily: 'inherit', fontWeight: 'inherit' } as React.CSSProperties,
        }
        if (s.active) {
          return (
            <span
              ...
              style={{ ...common.style, background: meta.color, fontSize: '1em' }}
            >
              {pinnedRedactionText(original)}
            </span>
          )
        }
        return (
