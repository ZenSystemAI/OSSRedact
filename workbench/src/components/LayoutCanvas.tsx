      ...
      if (span && span.active) {
        ...
        nodes.push(
          <span
            ...
            title={`${meta.en} · ${span.source} · ${span.rule}`}
            className={`tok tok-active${sel ? ' tok-selected' : ''}`}
            style={{ background: meta.color, ['--tok-color' as string]: meta.color } as React.CSSProperties}
          >
            {placeholderOf.get(span.id) ?? `<${span.label.toUpperCase()}>`}
          </span>,
        )
        i = j
