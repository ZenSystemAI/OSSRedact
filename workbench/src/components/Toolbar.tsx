...
export default function Toolbar(p: Props) {
  ...
  const exportDisabled = p.busy || !p.hasRedactions
  const resetLabel = p.busy
    ? 'Cancel the current operation and close this document'
    : 'Close this document'
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="relative" ref={menuRef}>
          <button
            ...
            aria-haspopup="menu"
            aria-expanded={menu}
            aria-controls="toolbar-export-menu"
          >
            Export
          ...
          {menu && (
            ...
            >
              <MenuItem label="Copy redacted text" hint="placeholders" disabled={exportDisabled} onClick={() => { p.onCopyRedacted(); setMenu(false) }} />
              <MenuItem label="Download redacted file" hint=".txt / .md / .csv" disabled={exportDisabled} onClick={() => { p.onDownloadRedacted(); setMenu(false) }} />
              <MenuItem label="Download entity map" hint=".json · sensitive · local" warn disabled={exportDisabled} onClick={() => { p.onDownloadMap(); setMenu(false) }} />
              <MenuItem label="Download audit trail" hint=".json · no values" disabled={exportDisabled} onClick={() => { p.onDownloadAudit(); setMenu(false) }} />
            </div>
          )}
        </div>
