 ...
 * remote gate requires one), Test, and Connect. The address is persisted via daemon.ts (localStorage);
 * the control token is session-only and must be re-entered after an application restart. Calls onChange
...
function GateConnection({ onChange }: { onChange: () => void }) {
  ...
  // Session-only: getControlToken() is empty after reload even if an address override remains.
  ...
  // Saved remote address but no session token: operator must reconnect after restart.
  const needsTokenReentry = isOverride && !getControlToken()
    ...
    <details className="mb-5 rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#191919]">
      ...
      <div className="space-y-3 border-t border-gray-100 dark:border-white/10 p-3.5">
        <p className="text-xs leading-relaxed text-gray-500 dark:text-neutral-400">
          ...
          The token is kept only for this session -- after restart, reconnect with the token again. A gate on
        ...
        {needsTokenReentry && (
          <p
            ...
            role="status"
          >
            A custom gate address is saved, but the control token is not retained across restarts. Paste the
            token and click <strong className="font-semibold">Connect</strong> to restore remote control.
          </p>
        )}
