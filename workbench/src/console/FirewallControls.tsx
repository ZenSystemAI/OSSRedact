...
import { firewallControl, routingConfig, waitForLocalFirewallReady, type FirewallStatus } from '../lib/firewall'
...
export default function FirewallControls({ onFirewallChange }: { onFirewallChange?: () => void }) {
  ...
  async function toggleFw() {
    ...
    try {
      ...
      // Stop returns as soon as systemd reports; start only after both services are healthy.
      await refresh()
      onFirewallChange?.()
    } catch (e) {
      if (
      ...
      try {
        ...
        // keep the readiness/start error already set
      }
    } finally {
      setBusy(null)
    }
