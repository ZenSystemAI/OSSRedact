...
import { loadPdfDoc, type PageGeom, type PageAssessment } from './pdf'
...
export async function loadFile(file: File): Promise<LoadedDoc> {
  ...
  if (ext === 'pdf') {
    const { text, pages, assess, bytes } = await loadPdfDoc(file)
    return { name: file.name, text, kind: 'pdf', pages, assess, bytes }
  }
...
// their value (the entity was removed on purpose), and net-new colleague text is untouched. Format-preserving
// for .docx/.xlsx (reuses the same rebuild() that put the placeholders in). PDF can't round-trip (the redacted
// PDF is image-only, no placeholders survive).
export async function rehydrateFile(
  file: File,
  map: EntityMap,
): Promise<{ blob: Blob; filename: string; restored: number; unknown: string[] }> {
  const ext = (file.name.split('.').pop() || '').toLowerCase()
  const base = file.name.replace(/\.[^.]+$/, '').replace(/^redacted-/, '')
  if (ext === 'pdf')
  ...
  const unknown = present.filter((ph) => !(ph in map))
  ...
  for (const ph of Object.keys(map)) {
    ...
    while (i !== -1) {
      repls.push({ start: i, end: i + ph.length, text: map[ph] })
  ...
  return { blob, filename: `restored-${base}.${outExt}`, restored, unknown }
}
